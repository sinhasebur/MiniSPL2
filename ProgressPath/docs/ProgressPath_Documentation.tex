...
\begin{enumerate}
  ...
  \item \code{GroqAssistantProvider} sends a \code{chat completions} request to the configured Groq model and returns the first text response.
...
The local project includes \code{.env.example}. Copy it to \code{.env} and set \code{GROQ\_API\_KEY}; \code{.env} is ignored by Git. The provider accepts the \code{GROQ\_API\_KEY} environment variable, or the \code{-Dgroq.api.key} JVM property. \code{GROQ\_MODEL} and \code{GROQ\_TIMEOUT\_SECONDS} are optional overrides. The default model is \code{openai/gpt-oss-20b} and the default request timeout is 45 seconds.
...
Before using the Assistant tab, copy \code{.env.example} to \code{.env} and set \code{GROQ\_API\_KEY}. The application reads this file itself; Maven does not need a dotenv plugin. Never commit the resulting \code{.env} file.
