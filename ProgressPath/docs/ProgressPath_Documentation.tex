...
Assistant & Select an active plan, ask a study question, and receive an asynchronous Groq Flash response based on a compact plan snapshot. The assistant does not edit stored data.\\
\bottomrule
\end{longtable}
...
\begin{itemize}
  ...
  \item \textbf{AI provider:} the controller calls the \code{AssistantProvider} boundary; \code{GroqFlashAssistantProvider} performs the network request without coupling Groq code to JavaFX or SQLite.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \code{GroqFlashAssistantProvider} sends a \code{generateContent} request to the configured Flash model and returns the first text response.
...
The local project includes \code{.env.example}. Copy it to \code{.env} and set \code{Groq\_API\_KEY}; \code{.env} is ignored by Git. The provider accepts the documented \code{GOOGLE\_API\_KEY} or \code{Groq\_API\_KEY} environment variables (\code{GOOGLE\_API\_KEY} takes precedence), or the \code{-Dgoogle.api.key} / \code{-DGroq.api.key} JVM properties. \code{Groq\_MODEL} and \code{Groq\_TIMEOUT\_SECONDS} are optional overrides. The default model is \code{Groq-3.6-flash} and the default request timeout is 45 seconds.

The HTTP client follows the Groq REST \code{models.generateContent} shape. It uses the \code{Authorization} header for authentication, keeping the key out of the request URL, and posts JSON containing a system instruction, one user content part, and a 512-token output limit.
...
This revision adds the eighth, Assistant tab; the provider interface, Groq Flash HTTP client, compact context builder, asynchronous JavaFX request handling, local \code{.env} loading, and the tracked \code{.env.example} template. Existing plan, course, progress, assignment, focus, analytics, persistence, and test workflows remain unchanged.
...
Provider Strategy & \code{AssistantProvider} with \code{GroqFlashAssistantProvider} & Isolates the AI vendor and HTTP details from the controller. A second provider or a local test double can be added without changing the Assistant screen.\\
\bottomrule
\end{longtable}
\normalsize
...
\code{ai/} & Assistant provider boundary, Groq Flash HTTP client, local configuration, and narrow response parsing.\\
